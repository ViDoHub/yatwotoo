...
from app.models import DealType, Listing, SavedSearch, ScrapeJob, UserSettings
...
def _build_scrape_params(filters: dict) -> tuple[dict, DealType, list[int] | None]:
    ...
    region_ids = None
    if top_area_ids := filters.get("top_area_ids"):
    ...
    for deal_type in [DealType.RENT, DealType.FORSALE]:
        for region_id in REGIONS:
            ...
            if len(listings) >= 200:
                ...
                listings = await fetch_all_listings(
                    {}, deal_type=deal_type, region_ids=[region_id], deep=True
                )
...
async def cleanup_stale_listings_job():
...
async def enrich_amenities_job(batch_size: int = 1000):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        for listing in listings:
            ...
            delay = settings.request_delay_min + (
                settings.request_delay_max - settings.request_delay_min
