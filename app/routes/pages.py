...
async def dashboard(request: Request):
    ...
    saved_searches = await SavedSearch.find(SavedSearch.is_active == True).to_list()
...
async def listings_page(request: Request):
    ...
    if request.headers.get("HX-Request"):
        return templates.TemplateResponse(request, "partials/listing_list.html", context={
            "listings": listings,
            "total": total,
            "page": page,
...
async def listing_detail(request: Request, yad2_id: str):
    ...
    price_history = (
        await PriceHistory.find(PriceHistory.listing_id == yad2_id)
        .sort([("observed_at", 1)])
        .to_list()
    )
