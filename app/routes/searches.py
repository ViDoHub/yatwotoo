...
async def test_whatsapp(request: Request):
    ...
    if not user_settings or not user_settings.whatsapp_phone:
        return HTMLResponse(
            '<span class="text-red-500">WhatsApp not configured</span>'
        )
    ...
    if success:
        ...
        return HTMLResponse(
            '<span class="text-red-500">Failed to send. Check phone number and API key.</span>'
        )
...
async def test_telegram(request: Request):
    ...
    if not user_settings or not user_settings.telegram_bot_token:
        return HTMLResponse(
            '<span class="text-red-500">Telegram not configured</span>'
        )
    ...
    if success:
        ...
        return HTMLResponse(
            '<span class="text-red-500">Failed to send. Check bot token and chat ID.</span>'
        )
...
async def test_email(request: Request):
    ...
    if not user_settings or not user_settings.email_smtp_host:
        return HTMLResponse(
            '<span class="text-red-500">Email not configured</span>'
        )
    ...
    if success:
        return HTMLResponse(
            '<span class="text-green-500">Email sent successfully!</span>'
        )
    else:
        return HTMLResponse(
            '<span class="text-red-500">Failed to send. Check SMTP settings.</span>'
        )
