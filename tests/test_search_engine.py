...
class TestSearchListings:
    ...
    async def test_search_with_rooms_filter(self):
        for rooms in [2, 3, 4, 5]:
            await Listing(
                yad2_id=f"rooms_{rooms}", rooms=float(rooms),
                deal_type=DealType.RENT, is_active=True,
            ).insert()
    ...
    async def test_search_with_price_filter(self):
        for price in [3000, 5000, 7000, 9000]:
            await Listing(
                yad2_id=f"price_{price}", price=price,
                deal_type=DealType.RENT, is_active=True,
            ).insert()
    ...
    async def test_search_with_amenity_filter(self):
        ...
        await Listing(
            yad2_id="has_parking", amenities=Amenities(parking=True),
            deal_type=DealType.RENT, is_active=True,
        ).insert()
        # Listing without parking
        await Listing(
            yad2_id="no_parking", amenities=Amenities(parking=False),
            deal_type=DealType.RENT, is_active=True,
        ).insert()
        # Listing with unknown parking (None) - excluded with strict match
        await Listing(
            yad2_id="unknown_parking", amenities=Amenities(parking=None),
            deal_type=DealType.RENT, is_active=True,
        ).insert()
    ...
    async def test_sort_by_price_asc(self):
        for price in [7000, 3000, 5000]:
            await Listing(
                yad2_id=f"sort_{price}", price=price,
                deal_type=DealType.RENT, is_active=True,
            ).insert()
...
class TestGetAreaCounts:
    ...
    async def test_area_counts_excludes_inactive(self):
        ...
        await Listing(
            yad2_id="inactive1",
            address=Address(area_id=10),
            deal_type=DealType.RENT, is_active=False,
        ).insert()
